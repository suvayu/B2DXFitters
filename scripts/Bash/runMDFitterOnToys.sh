...
while (( $seed < $stop )); do
    ...
    #Run script
    python ${pyscriptpath}runMDFitter_Bd.py -d --configName $config --inputFile root://eoslhcb.cern.ch/${eosinput}GenToyWorkspace_${nicknamegen}_${seed}.root --merge alreadyboth --decay Bd2DPi --pol both --mode kpipi --year run1 --hypo Bd2DPi_Bd2DK --sWeightsName ${output}sWeights_${nicknamemd}_${nicknamegen}_${seed}.root --dim 1 --binned --sWeights --toys --noFitPlot --pullFile root://eoslhcb.cern.ch/${eosoutput}PullTreeFitA_${nicknamemd}_${nicknamegen}_${seed}.root --pullFilesWeights root://eoslhcb.cern.ch/${eosoutput}PullTreeFitB_${nicknamemd}_${nicknamegen}_${seed}.root >& ${output}log_${nicknamemd}_${nicknamegen}_${seed}.txt  
