...
if __name__ == '__main__' :
    ...
    frame1 = time.frame(ROOT.RooFit.Title("D^{-}#pi^{+} OS inclusive"))
    frame2 = time.frame(ROOT.RooFit.Title("D^{-}#pi^{+} SS inclusive"))
    frame3 = time.frame(ROOT.RooFit.Title("D^{+}#pi^{-} OS inclusive"))
    frame4 = time.frame(ROOT.RooFit.Title("D^{+}#pi^{-} SS inclusive"))
    frame5 = time.frame(ROOT.RooFit.Title("D^{-}#pi^{+} OS exclusive"))
    frame6 = time.frame(ROOT.RooFit.Title("D^{-}#pi^{+} SS exclusive"))
    frame7 = time.frame(ROOT.RooFit.Title("D^{+}#pi^{-} OS exclusive"))
    frame8 = time.frame(ROOT.RooFit.Title("D^{+}#pi^{-} SS exclusive"))

    datasetOSinclPiplus = dataset.reduce("BacCharge==+1 && TagDecOS!=0")
    datasetSSinclPiplus = dataset.reduce("BacCharge==+1 && TagDecSS!=0")
    datasetOSinclPiminus = dataset.reduce("BacCharge==-1 && TagDecOS!=0")
    datasetSSinclPiminus = dataset.reduce("BacCharge==-1 && TagDecSS!=0")
    datasetOSexclPiplus = dataset.reduce("BacCharge==+1 && TagDecOS!=0 && TagDecSS==0")
    datasetSSexclPiplus = dataset.reduce("BacCharge==+1 && TagDecSS!=0 && TagDecOS==0")
    datasetOSexclPiminus = dataset.reduce("BacCharge==-1 && TagDecOS!=0 && TagDecSS==0")
    datasetSSexclPiminus = dataset.reduce("BacCharge==-1 && TagDecSS!=0 && TagDecOS==0")

    datasetOSinclPiplus.plotOn(frame1)
    datasetSSinclPiplus.plotOn(frame2)
    datasetOSinclPiminus.plotOn(frame3)
    datasetSSinclPiminus.plotOn(frame4)
    datasetOSexclPiplus.plotOn(frame5)
    datasetSSexclPiplus.plotOn(frame6)
    datasetOSexclPiminus.plotOn(frame7)
    datasetSSexclPiminus.plotOn(frame8)

    modelPDF.plotOn(frame1, ROOT.RooFit.ProjWData(datasetOSinclPiplus))
    modelPDF.plotOn(frame2, ROOT.RooFit.ProjWData(datasetSSinclPiplus))
    modelPDF.plotOn(frame3, ROOT.RooFit.ProjWData(datasetOSinclPiminus))
    modelPDF.plotOn(frame4, ROOT.RooFit.ProjWData(datasetSSinclPiminus))
    modelPDF.plotOn(frame5, ROOT.RooFit.ProjWData(datasetOSexclPiplus))
    modelPDF.plotOn(frame6, ROOT.RooFit.ProjWData(datasetSSexclPiplus))
    modelPDF.plotOn(frame7, ROOT.RooFit.ProjWData(datasetOSexclPiminus))
    modelPDF.plotOn(frame8, ROOT.RooFit.ProjWData(datasetSSexclPiminus))

    canvSplit = TCanvas("canvSplit")
    ...
    canvSplit.SaveAs(outTS.Data()+"chargeSplit.pdf")
