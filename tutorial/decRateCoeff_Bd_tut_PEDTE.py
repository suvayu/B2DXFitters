...
#        Manuel Schiller.
#       
#        Use case: per-event sigma_t, two per-event taggers combined, 
#                  "on the fly", realistic acceptance,
#                  asymmetries (prod, det, tageff)
#
# @usage python decRateCoeff_Bd_PEDTE.py SEED
#
...
# build mock decay time error distribution (~ timeerr^6 * exp(-timerr / (timerr_av / 7))
terrpdf_shape = WS(ws, RooConstVar('timeerr_ac', 'timeerr_ac', config['DecayTimeResolutionAvg'] / 7.))
terrpdf_truth = WS(ws, RooTruthModel('terrpdf_truth', 'terrpdf_truth', timeerr))
terrpdf_i0 = WS(ws, RooDecay('terrpdf_i0', 'terrpdf_i0', timeerr, terrpdf_shape, terrpdf_truth, RooDecay.SingleSided))
terrpdf_i1 = WS(ws, RooPolynomial('terrpdf_i1','terrpdf_i1', timeerr, RooArgList(zero, zero, zero, zero, zero, zero, one), 0))
...
plotAll(ds,fitpdf,qf,qt_os,time,"FitPdfAndDataOS_decRateCoeff_Bd_tut_PEDTE.eps")
plotAll(ds,fitpdf,qf,qt_ss,time,"FitPdfAndDataSS_decRateCoeff_Bd_tut_PEDTE.eps")
